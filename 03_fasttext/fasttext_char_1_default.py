...
model = fasttext.train_supervised(
    input = config.char_train_path, # 训练数据路径
    dim=10, # 词向量维度
    wordNgrams=2, # n-gram特征，默认为1，即不使用n-gram特征，设置为2表示使用2-gram特征
    epoch=5,
    lr=0.1
)
# 3.模型保存
# 模型保存路径设为ft_model_char_1.bin,
# ft_model.bin -> ft_model_char_1.bin
model.save_model(config.ft_model_path.replace(".bin", "_char_1.bin"))
print(f"模型已保存到: {config.ft_model_path.replace('.bin', '_char_1.bin')}")
# 4.模型评估
