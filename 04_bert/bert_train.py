...
from networkx.linalg.laplacianmatrix import total_spanning_tree_weight
...
def train_one_epoch(
    ...
    for batch in tqdm(train_dataloader, desc="Training"):
        ...
        y_pred_class = logits.argmax(dim=-1).to(device=torch.device('cpu')).tolist()    # (batch_size,)
...
def evaluate(
    ...
    """
    模型评估，并返回 损失、准确率、f1分数
    :param model: 模型对象
    :param dev_dataloader: 验证集数据加载器对象
    :param loss_fn: 损失函数对象
    :return: 验证损失、准确率、f1分数
    """
    # 1.设置为评估模式
    model.eval()
    # 2.初始化 总损失、总样本数、预测结果列表、真实标签列表
...
def train():
    ...
    for epoch in range(config.epochs):
        ...
        if val_f1 > best_f1:
            ...
            print(f"保存最优模型到文件: {config.bert_model_path}, 验证F1分数: {best_f1:.4f}")
